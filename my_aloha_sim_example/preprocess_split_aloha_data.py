...
def load_hdf5(demo_path):
    ...
    with h5py.File(demo_path, "r") as root:
        # 直接读取根目录的键
        qpos = root["observations/qpos"][()]  # (N, 14)
        action = root["action"][()]  # (N, 14)

        # 你没有 qvel 和 effort，可以填充为零
        ...
        image_dict = dict()
        for cam_id in [1, 2, 3]:
            cam_key = f"observations/images/cam{cam_id}"
            image_dict[f"cam{cam_id}"] = root[cam_key][()]  # uint8 (N, H, W, 3)

        # 假设是实机数据（不使用仿真），设为 False
        is_sim = False
...
def load_and_preprocess_all_episodes(demo_paths, out_dataset_dir, json_paths):
    """
    ...
    for demo in tqdm(demo_paths):
        ...
        # Save non-image info
        episode_len = image_dict["cam1"].shape[0]
        # Resize all images
        ...
        # 随机选择一个instruction
        all_instructions = seen_list + unseen_list
        random_instruction = random.choice(all_instructions)
...
# full_qvel, full_effort
def randomly_split(full_qpos, full_action, full_image_dict, full_qvel, full_effort,percent_val):
    ...
    train_dict = dict(
        ...
        # ),
        image_dict=dict(
            cam_high=shuffled_image_dict["cam1"][:num_episodes_train],
            cam_left_wrist=shuffled_image_dict["cam2"][:num_episodes_train],
            cam_right_wrist=shuffled_image_dict["cam3"][:num_episodes_train],
        ),
    )
    val_dict = dict(
        qpos=shuffled_qpos[num_episodes_train:],
...
def save_new_hdf5(out_dataset_dir, data_dict, episode_idx):
    ...
    H, W, C = data_dict["image_dict"]["cam1"][0].shape
    ...
    with h5py.File(
        ...
        _ = root.create_dataset("instruction", data=data_dict["instruction"], dtype=h5py.string_dtype(encoding='utf-8'))
        root["/instruction"][...] = data_dict["instruction"].encode('utf-8')  # Save instruction as bytes
...
def main(args):
    ...
    json_paths = glob.glob(os.path.join(args.instruction_path, "*.json"))

    load_and_preprocess_all_episodes(train_demo_paths, out_dataset_dir_train, json_paths)
    load_and_preprocess_all_episodes(val_demo_paths, out_dataset_dir_val, json_paths)
